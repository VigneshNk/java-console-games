...
public class Minesweeper {
	...
	public static int RevealSquare(int x, int y, int[][] player, int[][] hidden) {
		...
		if (player[y][x] == 1 || player[y][x] == 2 || player[y][x] == 3 || player[y][x] == 4 || hidden[y][x] == 5
				|| hidden[y][x] == 9 || hidden[y][x] == 8) {
	...
	public static void main(String[] args) {
		...
		System.out.println("You are presented with a 6 by 6 field that is filled with 8 mines and 3 hearts.");
		...
		while (count <= 10) {
			...
			while (wrongCheck == 0) {
				if (xR >= 1 && xR <= 6 && yR >= 1 && yR <= 6) {
					...
					// Wrong Input
					System.out.println("The numbers inputted does not fit the parmameters, please try again.");
					System.out.println();
		...
		System.out.println("Remeber you lose a life if you step on a mine, and gain a life if you find a heart.");
		System.out.println();

		// Heart at Starting Position
		if (hiddenField[1][1] == 7 || hiddenField[1][1] == 8) {
			System.out.println("Wow this is your lucky day, you have found an extra life at the starting position.");
